...
-- Q1.	Create DataBase BANK and Write SQL query to create above schema with constraints?
...
-- Q2.	Inserting Records into created tables Branch?
...
emp_no,branch_no,fname,mname,lname,dept
,desig,mngr_no)
values
...
-- Q 5.	Find the Date of Birth of customer name ‘Ramesh’?
...
select custid,min(curbal) as lowest_balance from account
group by custid
limit 1;

-- Q11.	Give the count of customer for each occupation?
select occupation,count(occupation) as no_of_customer from customer 
group by occupation;

-- Q12.	Write a query to find the name (first_name, last_name) of the employees who are managers?.
select fname,mname from employee where desig like '%manager%';

-- Q13.	List name of all employees whose name ends with a?
select fname,mname,lname from employee where lname like '%a';
...
-- Q15.	Write a query to display the customer number, customer firstname, account number for the customer’s who are born after 15th of any month?
select c.custid,c.fname,a.acnumber from customer c join account a on c.custid=a.custid
where day(c.dob) > 15;
...
select c.custid,c.fname,a.bid as branch_id,a.curbal from customer c join account a 
...
where bid in(select bid from branch_mstr)));
...
insert into transaction(tid,custid,account_no,bid,amount,type,DOT)
values
(1,1,1,1,1000,'Deposit','2023-02-03'),
(2,2,2,2,3000,'Deposit','2023-02-03');

-- Q19.	Write a query to display the details of customer with second highest balance?
select * from 
(select *, rank() over(order by a.curbal desc) as ranking from
(select c.custid,c.fname,c.mname,c.lname,c.occupation,c.dob,a.curbal from customer c
join account a on (c.custid=a.custid))t)tt
where ranking=2;
...
Insert into ONLINE_CUSTOMER (CUSTOMER_ID,CUSTOMER_FNAME,CUSTOMER_LNAME,CUSTOMER_EMAIL,CUSTOMER_PHONE,ADDRESS_ID,CUSTOMER_CREATION_DATE,CUSTOMER_USERNAME,CUSTOMER_GENDER)
values (1,'Jennifer','Wilson','jen_w@gmail.com',9776363306,909,str_to_date('01-JUN-91','%d-%b-%y'),'jen_w','F');
Insert into ONLINE_CUSTOMER (CUSTOMER_ID,CUSTOMER_FNAME,CUSTOMER_LNAME,CUSTOMER_EMAIL,CUSTOMER_PHONE,ADDRESS_ID,CUSTOMER_CREATION_DATE,CUSTOMER_USERNAME,CUSTOMER_GENDER)
values (2,'Jackson','Davis','dave_jack@gmail.com',9886363307,910,str_to_date('12-JUN-01','%d-%b-%y'),'dave_jack','M');
...
Insert into ONLINE_CUSTOMER (CUSTOMER_ID,CUSTOMER_FNAME,CUSTOMER_LNAME,CUSTOMER_EMAIL,CUSTOMER_PHONE,ADDRESS_ID,CUSTOMER_CREATION_DATE,CUSTOMER_USERNAME,CUSTOMER_GENDER)
values (10,'Bidhan','C.Roy','bidhanroy@yahoo.co.in',9886218583,918,str_to_date('23-OCT-11','%d-%b-%y'),'bcroy','F');
...
/* Q1. Display the product details as per the following criteria and sort them in descending order of category:
-- a.  If the category is 2050, increase the price by 2000
-- b.  If the category is 2051, increase the price by 500
-- c.  If the category is 2052, increase the price by 600

*/
select product_desc, product_class_code,
(case
...
from product
where product_class_code in (2050,2051,2052)
order by product_class_code desc;


-- Q2. List the product description, class description and price of all products which are shipped. */

select p.product_desc, p.product_class_code, pc.product_class_desc
from product p
...
-- Q3.Show inventory status of products as below as per their available quantity:
...
(case
when pc.product_class_desc like '%Electronics%' or pc.product_class_desc like '%Computer%' and p.product_quantity_avail <= 10 then 'Low Stock'
...
where product_id in (select product_id from product where product_class_code in
(select product_class_code from product_class where product_class_desc in ('Toys','Books')))));
